...
\begin{figure}[ht]
   \centering
   \includegraphics[width=0.45\textwidth]{imgs/model.png}
...
Though Energat is more computationally expensive and could lead to resource contention, for collecting accurate energy values, it was the most practical tool to incorporate into our system. To alleviate the issues associated with EnergyLiteDaemon, possible future steps that could be taken to improve the tool include reducing the energy sampling period and refining the interpolation technique used, and settling on a middle ground of having a separate process for each set of 20 or so containers, rather than having only 1 process or having 1 process per container. 
...
For the rest of the evaluation, we used Energat as our energy measurement tool and evaluated the efficacy of our end-to-end system in terms of conserving energy while meeting SLOs. 

\subsubsection{Energy Comparison}
For a given specific function type, we add up the energy readings for each invocation that falls under the function and use that as the metric for the total amount of energy consumed for a given function. Figures {x} and {y} show the comparison between the Carbon Aware Serveless System (CASS) and Aquatope, as well as CASS and Parrotfish, respectively. 
